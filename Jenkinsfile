node{
    ...
    stage("Terraform variables"){
        withCredentials([string(credentialsId: 'aws-access-key', variable: 'access'), string(credentialsId: 'aws-secret-key', variable: 'secret')]) {
          sh "sed 's/ACCESS/${access}/g' terraform/terraform.tfvars > terraform/terraform.tfvars"
          sh "sed 's/SECRET/${secret}/g' terraform/terraform.tfvars > terraform/terraform.tfvars"
        }
